        ...
        with st.container(border=True):
            ...
            with col2:
                ...
                if copy_button_pressed:
                    ...
                    st.toast(f":material/check_circle: **{selected_example['name']}** copied to workspace!", icon=":material/content_copy:")
                    st.rerun()
...
def _render_workspace_header(*, example_is_loaded: bool) -> None:
    ...
    with col1:
        ...
        if example_is_loaded and "example_name" in st.session_state:
            st.caption(f":material/lightbulb: Using example: **{st.session_state.example_name}**")
    ...
    with st.form("problem_form", border=False):
        ...
        if solve_button_pressed:
            if problem_text.strip():
                ...
                st.toast(":material/calculate: Processing your optimization problem...", icon=":material/play_arrow:")
                st.rerun()
            else:
                st.toast(":material/warning: Please enter a problem to solve!", icon=":material/error:")
        
        if clear_button_pressed:
            _clear_workspace_session_state()
            st.toast(":material/refresh: Workspace cleared successfully!", icon=":material/restart_alt:")
            st.rerun()
...
def _detect_variable_type_from_text(*, problem_text: str) -> str:
    ...
    return "integer" if any(keyword in text_lower for keyword in integer_keywords) else "continuous"
...
def _render_thesis_header_with_download(*, thesis_file_path: Path) -> None:
    ...
    with col1:
        st.markdown("**:material/school: Research Thesis - SolvedOR: Modern Operations Research Interface**")
        st.caption("This thesis introduces the OR-Solver application and its methodology.")
    ...
    with col2:
        try:
            with open(thesis_file_path, "rb") as pdf_file:
                ...
                if download_pressed:
                    st.toast(":material/file_download_done: Thesis PDF ready for download!", icon=":material/download:")
        except OSError:
...
def _render_pdf_viewer(*, pdf_file_path: Path) -> None:
    ...
    try:
        ...
        st.warning("PDF viewer not available. You can download the thesis using the button above.")
        st.info(f"Error: {str(pdf_error)}")


def _render_readme_tab() -> None:
    """Render the project README tab."""
    _render_markdown_file(file_name="README.md", not_found_message="README.md not found.")


def _render_contributing_tab() -> None:
    """Render the contributing guide tab."""
    _render_markdown_file(file_name="CONTRIBUTING.md", not_found_message="CONTRIBUTING.md not found.")
