...
def _solve_linear_program(problem, variable_type="continuous"):
    ...
    for var_name in variables:
        if var_name in problem.variables:
            ...
            upper = (
                float(var_obj.upper_bound) if var_obj.upper_bound is not None else None
            )
...
def _display_results(problem, translations, variable_type="continuous"):
    ...
    with st.container(border=True):
        ...
        with metric_cols[3]:
            st.metric(
                translations.results.constraints, len(problem.constraints), delta=None
            )
        ...
        with summary_cols[0]:
            st.metric(
                translations.results.summary.objective,
                problem.objective.direction.value,
            )

        with summary_cols[1]:
            st.metric(
                translations.results.summary.status,
                translations.results.summary.optimal,
            )
        ...
        _render_visualizations(
            problem, num_vars, solution_data, translations, variable_type
        )


def _render_visualizations(
    problem, num_vars, solution_data, translations, variable_type="continuous"
):
    ...
    if num_vars == 2:
        st.success(
            ":material/target: **2D Visualization Available** - Interactive feasible region and solution display"
        )
        ...
        st.success(
            ":material/auto_awesome: **3D Visualization Available** - Interactive 3D feasible region exploration"
        )
        ...
        st.info(
            f":material/info: **Visualization Info:** This problem has {num_vars} variables. Interactive visualizations are available for 2D and 3D problems only."
        )
...
def _create_2d_visualization(solution_data, translations, variable_type="continuous"):
    ...
    var1_name = (
        solution_data["Variable"][0] if len(solution_data["Variable"]) > 0 else "x₁"
    )
    var2_name = (
        solution_data["Variable"][1] if len(solution_data["Variable"]) > 1 else "x₂"
    )
    ...
    if (
        start_x >= 0
        and start_y >= 0
        and start_x + 2 * start_y <= 8
        and 2 * start_x + start_y <= 8
    ):
        end_x = start_x + 0.5
    ...
    fig.add_annotation(
        x=0.2,
        y=4.7,
        text="📈 Gradient ∇f = (3, 2)<br>Arrow shows optimization direction",
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode="markers",
            marker=dict(
                symbol="star", size=20, color="gold", line=dict(color="black", width=2)
            ),
...
def _create_3d_visualization(solution_data, translations):
    ...
    var3_name = (
        solution_data["Variable"][2] if len(solution_data["Variable"]) > 2 else "x₃"
    )
